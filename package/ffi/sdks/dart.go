...
// buildPackage builds a single Dart package
func (s *DartSDK) buildPackage(ctx context.Context, client *dagger.Client, container *dagger.Container, hostDirectory *dagger.Directory, opts BuildOpts, packageName string, includePatterns []string) error {

	container = container.From("ghcr.io/cirruslabs/flutter:stable").
		WithDirectory("/src", hostDirectory.Directory(packageName), dagger.ContainerWithDirectoryOpts{
...
func (s *DartSDK) Build(ctx context.Context, client *dagger.Client, container *dagger.Container, hostDirectory *dagger.Directory, opts BuildOpts) error {
	...
	// Define includes for each package
	packages := []struct {
		name     string
		includes []string
	}{
		{
			name: "flipt-client-dart",
			includes: append([]string{
				"**/native/linux_x86_64/**",
				"**/native/linux_aarch64/**",
				"**/native/darwin_aarch64/**",
				"**/native/windows_x86_64/**",
			}, dynamicInclude...),
		},
		{
			name: "flipt-client-dart-android",
			includes: append([]string{
				"**/android/**",
			}, dynamicInclude...),
		},
		{
			name: "flipt-client-dart-ios",
			includes: append([]string{
	...
	for _, pkg := range packages {
		...
		if err := s.buildPackage(ctx, client, container, hostDirectory, opts, pkg.name, pkg.includes); err != nil {
